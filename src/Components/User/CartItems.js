...
import { addProductIntoWishlist, deleteAllProductsFromCart, deleteProductFromCart, fetchCartItems, removeAllProductsFromCart, removeProductFromCart } from "../../DataSlice/ProductSlice";
...
export default function CartItems() {
    ...
    return <>
        ...
        {cartItems.length != 0 ? (
            <section className="row container-fluid" style={{ backgroundColor: "#F7FAFC" }}>
                <div id="cart-left" className="col-md-9">
                    {cartItems?.map((cart, index) => <div key={index} className="row p-4 d-flex border" style={{ width: '100%', backgroundColor: 'white', height: 'auto' }}>
                        ...
                        <div  style={{ width: '50%', height: 'auto' }}>
                            ...
                            <p className="mb-1 text-secondary">{cart["products.description"].slice(0, 100)}</p>
                            {/* <div className="p-1"> */}
                            <button onClick={() => removeItemFromCart(index, cart["products.id"])} className="m-2 btn btn-outline-danger">Remove</button>
                            &nbsp;&nbsp;<button onClick={() => addToWishlist(cart['products.id'])} className="btn btn-outline-primary m-2">Save For Later</button>
                            {/* </div> */}
                        </div>
                        <div style={{ width: '220px', height: '100px' }} className="border">
                            ...
                            {/* <input type="number" defaultValue={(event)=>setQuantity(event.target.value)} min={1} onChange={()=>updateQty(index)} style={{width:'60px'}}/> */}
                        </div>
                    </div>)}
                ...
                <div id="cart-right" className="col-md-3">
